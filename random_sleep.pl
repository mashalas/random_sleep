...
my $seconds_duration = -1; # если неверный формат (переданный параметр не €вл€етс€ числом с одним из суффиксов smhd соответствующих second/minute/hour/day, то функци€ возвратит -1
my ($value, $measure);
my $measure_size = 1;

  if ($raw_duration =~ /^(\d+)([smhd]?)$/i) {
    # указана единица измерени€
    $value = $1;
    $value *= 1;
    $measure = $2;
    $measure = lc($measure);
...
my ($min_intervals_count, $max_intervals_count, $intervals_count);
  ...
  if ($min_duration > $max_duration) {
    # помен€ть пор€док очерЄдность минимальной и максимвльной продолжительности (сначала должна быть минимальна€)
  ...
  if ($verbose eq "v" || $verbose eq "verbose") {
    #если подробный режим - вывести сколько интервалов ожидани€ по 1 единице измерени€ и сколько это всего займЄт секунд
    my $total_sleep_duration_seconds = $intervals_count * $measure_size; # обща€ продолжительность сна в секундах
  ...
  if ($verbose eq "verbose") {
    # ввести врем€, когда закончили сон, если включен очень расширенный режим
    $s = "[" . get_timestamp() . "]";
    $s .= "\t" . "complete";
    print "$s\n";
  }
